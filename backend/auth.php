...
class Auth
{
    ...
    public static function register($username, $password, $fullname, $dob, $address, $gender, $email, $phone)
    {
        ...
        if (Auth::checkExist("username", $username) && Auth::checkExist("email", $email) && Auth::checkExist("phone", $username)) {
            $sqllast = "SELECT code FROM `user` order by id DESC limit 1";
            $lastCode = mysqli_query($conn, $sqllast)->fetch_assoc();
//            echo print_r($lastCode, true);
            $numCode = substr($lastCode['code'], -3) + 1;
            $code = 'THOD' . $numCode;

            $sql = "insert into user(username,password,fullname,dob,address,gender,email,phone,role,code)" .
                " values('$username','" . md5($password) . "','$fullname','$dob','$address',$gender,'$email','$phone',0,'$code')";
            mysqli_query($conn, $sql);
            Auth::findOneByUsernameAndPassword($username, md5($password));
